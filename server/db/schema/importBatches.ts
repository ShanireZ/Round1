...
export const importBatches = pgTable(
  ...
  (t) => [
    check(
      "import_batches_bundle_type_check",
      sql`${t.bundleType} IN ('question_bundle', 'prebuilt_paper_bundle', 'manual_question_import')`,
    ),
    check(
      "import_batches_status_check",
      sql`${t.status} IN ('dry_run', 'processing', 'applied', 'partial_failed', 'failed')`,
    ),
