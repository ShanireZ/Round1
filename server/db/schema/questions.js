...
import { pgTable, uuid, text, integer, jsonb, varchar, boolean, timestamp, index, check, } from "drizzle-orm/pg-core";
...
export const questions = pgTable("questions", {
    ...
    check("questions_type_check", sql `${t.type} IN ('single_choice', 'reading_program', 'completion_program')`),
    check("questions_difficulty_check", sql `${t.difficulty} IN ('easy', 'medium', 'hard')`),
    check("questions_status_check", sql `${t.status} IN ('draft', 'reviewed', 'published', 'archived')`),
