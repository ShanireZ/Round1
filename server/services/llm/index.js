...
import { createProviderLanguageModel, getProviderReasoningHistoryOptions, getProviderReasoningAttempts, getProviderReasoningSummaryOptions, getTaskExecutionChain, getTaskReasoningPolicy, getTaskReasoningSummary, isReasoningRetryableError, mergeProviderOptions, } from "../../../config/llm.js";
...
function parseObjectFromProviderErrorText(error, schema) {
...
function resolveResponseEnvelope(result) {
...
function resolveFinishReason(result) {
    return (getString(asRecord(asRecord(result.response)?.body)?.finishReason) ??
        getString(result.finishReason));
}
function resolveResponseId(result) {
    return getString(resolveResponseEnvelope(result)?.id);
}
function resolveResponseModel(result, fallbackModel) {
...
function resolveResponseMessages(result) {
    const responseRecord = asRecord(result.response);
    return Array.isArray(responseRecord?.messages)
        ? responseRecord.messages
        : undefined;
}
function buildConversationMessages(prompt, messages) {
...
function hasReasoningHistory(messages) {
    if (!messages || messages.length === 0) {
        return false;
    }
...
function describeReasoningAttempt(attempt) {
    return attempt.resolvedValue ?? "default";
}
function describeEntry(entry) {
    return `${entry.providerName}:${entry.model}`;
}
function buildProviderOptions(task, entry, attempt, hasConversationReasoningHistory = false) {
    return mergeProviderOptions(getProviderReasoningSummaryOptions(entry.providerName, getTaskReasoningSummary(task), entry.model), attempt.providerOptions, getProviderReasoningHistoryOptions(entry.providerName, hasConversationReasoningHistory, entry.model));
}
async function executeWithReasoningFallback(params) {
    let lastError;
    ...
    for (const [entryIndex, entry] of params.chain.entries()) {
        ...
        for (const [attemptIndex, attempt] of attempts.entries()) {
            ...
            try {
                const result = await params.execute(entry, buildProviderOptions(params.task, entry, attempt, params.hasReasoningHistory ?? false));
                ...
                await logLlmProviderCall({
                    ...
                    latencyMs,
                    costEstimate: estimateLlmCost(result.responseModel ?? result.model, result.tokensIn, result.tokensOut),
                ...
                const shouldRetryReasoning = isReasoningRetryableError(err) && attemptIndex < attempts.length - 1;
 ...
 */
export async function llmGenerateObject(params) {
    ...
    const result = await executeWithReasoningFallback({
        ...
        execute: async (entry, providerOptions) => {
            ...
            try {
                ...
                const usage = resolveUsage(error);
                return {
                    ...
                    finishReason: resolveFinishReason(error),
                    responseId: resolveResponseId(error),
                    responseModel: resolveResponseModel(error, entry.model),
                    reasoningText: resolveReasoningText(error),
                    warningsJson: [
                        {
