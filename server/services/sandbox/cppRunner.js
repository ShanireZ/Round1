 ...
 */
import { sandboxConfig } from '../../../config/sandbox.js';
 ...
 * 编译并运行 C++ 代码
 */
export async function runCpp(request) {
...
export async function verifyCpp(params) {
    ...
    for (let i = 0; i < params.sampleInputs.length; i++) {
        ...
        if (expected && actual !== expected) {
            logger.info({ testCase: i, expected, actual }, 'Output mismatch in sandbox verify');
            return { verified: false, results };
        }
    }
