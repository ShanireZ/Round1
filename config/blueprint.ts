 ...
 * - NOI 大纲 2025 修订版
 *
 * spec_json 接口定义见 reference-schema.md#spec_json-蓝图接口定义
 */
...
export interface BlueprintSection {
  ...
  subQuestionsPerGroup?: number;
  groupCount?: number;
  pointsPerQuestion: number;
  maxScore: number;
  difficultyDistribution: Record<string, number>;
  primaryKpQuota: { kpCode: string; count: number }[];
}
...
function cspJSections(kpQuotas: {
  sc: { kpCode: string; count: number }[];
  rp: { kpCode: string; count: number }[];
  cp: { kpCode: string; count: number }[];
  diffSc?: Record<string, number>;
  diffRp?: Record<string, number>;
  diffCp?: Record<string, number>;
}): BlueprintSection[] {
  return [
    {
...
export const blueprintSpecs: Record<ExamType, BlueprintSpec> = {
  "CSP-J": {
    examType: "CSP-J",
    durationMinutes: 120,
