...
export function minimumArchetypeCountForExam(examType: ExamType, standardMinimum = 12): number {
  if (examType === "GESP-1") return Math.min(3, standardMinimum);
  if (examType === "GESP-2") return Math.min(6, standardMinimum);
...
function buildGroup(kpGroup: KpGroup, seeds: ArchetypeSeed[]): QuestionArchetype[] {
  return seeds.map((seed) => ({
    ...
    examCoverage: examCoverage(seed.minGespLevel, seed.noiTier),
...
export const questionArchetypes: QuestionArchetype[] = [
  ...buildGroup("DS", [
    ...
    {
      suffix: "vector-push-pop-size",
      minGespLevel: 5,
      noiTier: "entry",
      taskFlavor: "vector_size_capacity_trace",
      codeStructureTags: ["container-simulation", "loop"],
      containerTags: ["vector"],
      feature: "vector push_back/pop_back 后 size 变化",
      trapType: "size_value_confusion",
      promptDirective: "必须考 vector push_back/pop_back/size 的状态变化，区分元素值与容器长度。",
    },
    {
      suffix: "vector-resize-default",
      minGespLevel: 5,
      noiTier: "entry",
      taskFlavor: "vector_resize_default",
      codeStructureTags: ["container-simulation", "mutation"],
      containerTags: ["vector"],
      feature: "vector resize 默认值与截断",
      trapType: "default_initialization_confusion",
      promptDirective: "必须考 vector resize 扩容默认值或缩短截断后的内容。",
    },
    {
      suffix: "array-stack-simulation",
      minGespLevel: 5,
      noiTier: "entry",
      taskFlavor: "array_stack_simulation",
      codeStructureTags: ["array", "container-simulation"],
      containerTags: ["array", "stack"],
      feature: "数组模拟栈的 top 指针变化",
      trapType: "top_index_off_by_one",
      promptDirective: "必须考数组模拟栈时 top 下标的增减和当前栈顶元素。",
    },
    {
      suffix: "array-queue-simulation",
      minGespLevel: 5,
      noiTier: "entry",
      taskFlavor: "array_queue_simulation",
      codeStructureTags: ["array", "container-simulation"],
      containerTags: ["array", "queue"],
      feature: "数组模拟队列的 head/tail 变化",
      trapType: "head_tail_boundary",
      promptDirective: "必须考数组模拟队列时 head/tail 的移动和队首元素。",
    },
    {
      suffix: "circular-queue-index",
      minGespLevel: 5,
      noiTier: "entry",
      taskFlavor: "circular_queue_index",
      codeStructureTags: ["array", "modulo", "container-simulation"],
      containerTags: ["array", "queue"],
      feature: "循环队列取模下标变化",
      trapType: "wraparound_off_by_one",
      promptDirective: "必须考循环队列 head/tail 取模回绕后的下标或元素顺序。",
    },
    {
      suffix: "two-stacks-transfer",
      minGespLevel: 5,
      noiTier: "entry",
      taskFlavor: "two_stacks_transfer",
      codeStructureTags: ["container-simulation", "branch"],
      containerTags: ["stack"],
      feature: "两个栈之间转移元素的顺序",
      trapType: "transfer_reverses_order",
      promptDirective: "必须考两个 stack 之间倒入元素后顺序被反转的结果。",
    },
    {
      suffix: "queue-filter-process",
      minGespLevel: 5,
      noiTier: "entry",
      taskFlavor: "queue_filter_process",
      codeStructureTags: ["container-simulation", "branch"],
      containerTags: ["queue"],
      feature: "队列按条件处理并重新入队",
      trapType: "processed_item_reenqueue_confusion",
      promptDirective: "必须考 queue 中元素按条件出队、丢弃或重新入队后的队列状态。",
    },
    {
      suffix: "vector-pair-sort",
      minGespLevel: 5,
      noiTier: "entry",
      taskFlavor: "vector_pair_sort_order",
      codeStructureTags: ["array", "sort", "record"],
      containerTags: ["vector", "pair"],
      feature: "vector<pair> 排序后的字典序",
      trapType: "tie_break_order_confusion",
      promptDirective: "必须考 vector<pair<int,int>> 默认排序或给定比较器后的顺序。",
    },
    {
      suffix: "sequence-adjacent-swap",
      minGespLevel: 5,
      noiTier: "entry",
      taskFlavor: "sequence_adjacent_swap",
      codeStructureTags: ["array", "mutation"],
      containerTags: ["array", "vector"],
      feature: "顺序表相邻交换后的下标和值",
      trapType: "index_after_swap_confusion",
      promptDirective: "必须考数组或 vector 中相邻元素交换后的下标和值，不得只问最终长度。",
    },
    {
  ...
  ...buildGroup("ALG", [
    ...
    {
      suffix: "linear-scan-extreme",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "linear_scan_extreme",
      codeStructureTags: ["array", "loop", "branch"],
      feature: "线性扫描最大值或最小值",
      trapType: "initial_value_boundary",
      promptDirective: "必须考数组线性扫描最大/最小值，包含初值或相等元素边界。",
    },
    {
      suffix: "conditional-count",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "conditional_count",
      codeStructureTags: ["loop", "branch"],
      feature: "按条件计数",
      trapType: "strict_vs_non_strict_condition",
      promptDirective: "必须考循环中按条件计数，条件要包含严格/非严格边界。",
    },
    {
      suffix: "running-sum-average",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "running_sum_average",
      codeStructureTags: ["loop", "accumulator"],
      feature: "累加器与平均值",
      trapType: "integer_division_after_sum",
      promptDirective: "必须考累加求和后再计算平均值或比例，明确整数除法语义。",
    },
    {
      suffix: "nested-loop-pair-count",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "nested_loop_pair_count",
      codeStructureTags: ["nested-loop", "counting"],
      feature: "双重循环枚举二元组",
      trapType: "self_pair_or_ordered_pair_confusion",
      promptDirective: "必须考双重循环枚举二元组的计数，明确是否允许 i=j 以及是否区分顺序。",
    },
    {
      suffix: "simple-simulation-state",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "simple_state_simulation",
      codeStructureTags: ["loop", "branch", "state-machine"],
      feature: "简单状态模拟",
      trapType: "state_update_order",
      promptDirective: "必须考循环中 2 个状态量按顺序更新后的结果，不能退化成单变量累加。",
    },
    {
      suffix: "sentinel-loop-stop",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "sentinel_loop_stop",
      codeStructureTags: ["loop", "branch"],
      feature: "哨兵条件终止循环",
      trapType: "stop_value_processed_or_not",
      promptDirective: "必须考遇到哨兵值时循环是否处理当前值以及最终状态。",
    },
    {
      suffix: "array-reverse-swap",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "array_reverse_swap",
      codeStructureTags: ["array", "two-pointer", "mutation"],
      feature: "数组原地反转交换",
      trapType: "swap_order_or_middle_element",
      promptDirective: "必须考数组小规模原地反转或成对交换后的中间/最终状态。",
    },
    {
      suffix: "selection-max-index",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "selection_index_trace",
      codeStructureTags: ["array", "loop", "branch"],
      feature: "选择最大值下标",
      trapType: "first_last_tie_break",
      promptDirective: "必须考寻找最大/最小值下标时相等元素保留第一次还是最后一次。",
    },
    {
      suffix: "frequency-small-array",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "frequency_array_count",
      codeStructureTags: ["array", "counting"],
      feature: "小范围频次数组",
      trapType: "index_value_confusion",
      promptDirective: "必须考小范围整数频次数组的下标含义和查询结果。",
    },
    {
      suffix: "merge-two-sorted-short",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "merge_two_sorted_trace",
      codeStructureTags: ["array", "two-pointer"],
      feature: "合并两个有序短数组",
      trapType: "remaining_tail_omitted",
      promptDirective: "必须考两个有序短数组合并过程，包含一边耗尽后的剩余元素处理。",
    },
    {
      suffix: "loop-invariant-accumulator",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "loop_invariant_accumulator",
      codeStructureTags: ["loop", "invariant"],
      feature: "循环不变量与累加器关系",
      trapType: "pre_post_update_confusion",
      promptDirective: "必须考每轮循环前后累加器含义的变化，选项要区分更新前/更新后。",
    },
    {
  ...
  ...buildGroup("CPP", [
    ...
    {
      suffix: "arithmetic-assignment-trace",
      minGespLevel: 1,
      noiTier: "entry",
      taskFlavor: "arithmetic_assignment_trace",
      codeStructureTags: ["expression", "assignment"],
      feature: "算术表达式与赋值顺序",
      trapType: "assignment_order_confusion",
      promptDirective: "必须考简单算术表达式和赋值语句执行后的变量值。",
    },
    {
      suffix: "cout-format-order",
      minGespLevel: 1,
      noiTier: "entry",
      taskFlavor: "cout_output_order",
      codeStructureTags: ["io", "expression"],
      feature: "cout 连续输出顺序",
      trapType: "separator_or_newline_confusion",
      promptDirective: "必须考 cout 连续输出时字符、空格或换行的确切顺序。",
    },
    {
      suffix: "integer-division-modulo",
      minGespLevel: 2,
      noiTier: "entry",
      taskFlavor: "integer_division_modulo",
      codeStructureTags: ["expression", "modulo"],
      feature: "整数除法与取模",
      trapType: "division_remainder_confusion",
      promptDirective: "必须考 int 除法截断和 % 余数的确定性结果。",
    },
    {
      suffix: "compound-assignment-step",
      minGespLevel: 2,
      noiTier: "entry",
      taskFlavor: "compound_assignment_trace",
      codeStructureTags: ["expression", "assignment"],
      feature: "复合赋值逐步更新",
      trapType: "old_value_vs_new_value",
      promptDirective: "必须考 +=、-=、*= 或 %= 的逐步更新，避免未指定求值顺序。",
    },
    {
      suffix: "array-index-update",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "array_index_update",
      codeStructureTags: ["array", "mutation"],
      containerTags: ["array"],
      feature: "数组下标访问与修改",
      trapType: "zero_based_index_confusion",
      promptDirective: "必须考数组从 0 开始的下标访问和修改后的元素值。",
    },
    {
      suffix: "for-while-control-flow",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "loop_control_flow_trace",
      codeStructureTags: ["loop", "branch"],
      feature: "for/while 控制流",
      trapType: "break_continue_position",
      promptDirective: "必须考 break 或 continue 对循环状态和输出顺序的影响。",
    },
    {
      suffix: "function-return-branch",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "function_return_branch",
      codeStructureTags: ["function", "branch"],
      feature: "函数返回值与分支",
      trapType: "early_return_confusion",
      promptDirective: "必须考函数中分支和提前 return 对返回值的影响。",
    },
    {
  ...
  ...buildGroup("MATH", [
    ...
    {
      suffix: "parity-invariant",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "parity_invariant",
      codeStructureTags: ["number-theory", "parity"],
      feature: "奇偶性不变量",
      trapType: "operation_changes_parity",
      promptDirective: "必须考加减乘或简单操作后奇偶性是否改变。",
    },
    {
      suffix: "divisibility-rule",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "divisibility_rule",
      codeStructureTags: ["number-theory", "modulo"],
      feature: "整除规则与余数",
      trapType: "divisible_vs_remainder",
      promptDirective: "必须考整除判断或余数分类，给出可手算的小整数。",
    },
    {
      suffix: "quotient-remainder",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "quotient_remainder_reasoning",
      codeStructureTags: ["number-theory", "division"],
      feature: "商和余数关系",
      trapType: "quotient_remainder_swapped",
      promptDirective: "必须考 a = bq + r 且 0 <= r < b 的约束。",
    },
    {
      suffix: "digit-sum",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "digit_sum_trace",
      codeStructureTags: ["number-theory", "loop"],
      feature: "各位数字和",
      trapType: "leading_zero_or_last_digit",
      promptDirective: "必须考短整数拆位和数字和，明确末位与高位处理顺序。",
    },
    {
      suffix: "minmax-expression",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "minmax_expression",
      codeStructureTags: ["comparison", "branch"],
      feature: "最大最小值表达式",
      trapType: "inclusive_boundary_comparison",
      promptDirective: "必须考 max/min 或条件比较后的确定值，包含相等边界。",
    },
    {
      suffix: "ceil-floor-integer",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "ceil_floor_integer",
      codeStructureTags: ["division", "boundary"],
      feature: "整数上取整与下取整",
      trapType: "ceil_formula_off_by_one",
      promptDirective: "必须考正整数除法上取整/下取整公式及整除边界。",
    },
    {
      suffix: "factor-multiple-basic",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "factor_multiple_basic",
      codeStructureTags: ["number-theory", "enumeration"],
      feature: "因数与倍数枚举",
      trapType: "include_one_or_self",
      promptDirective: "必须考小整数因数或倍数枚举，明确是否包含 1 和自身。",
    },
    {
      suffix: "small-enumeration-count",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "small_enumeration_count",
      codeStructureTags: ["counting", "enumeration"],
      feature: "小规模枚举计数",
      trapType: "duplicate_case_counted",
      promptDirective: "必须考小范围枚举计数，明确是否去重以及顺序是否重要。",
    },
    {
      suffix: "prime-sieve",
      minGespLevel: 5,
  ...
  ...buildGroup("BAS", [
    ...
    },
    {
      ...
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "table_trace_reasoning",
      codeStructureTags: ["table", "state-trace"],
      feature: "表格化状态追踪",
      trapType: "row_column_state_confusion",
      promptDirective: "必须给出 3-5 行状态表或伪代码表格，考下一行状态或最终状态。",
    },
  ]),
  ...buildGroup("CS", [
    {
    ...
    {
      suffix: "data-sort-filter",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "data_sort_filter_concept",
      codeStructureTags: ["data", "table", "sort"],
      feature: "表格数据排序与筛选",
      trapType: "sort_key_filter_order",
      promptDirective: "必须考表格数据按条件筛选或按字段排序后的结果。",
    },
    {
      suffix: "binary-unit-estimate",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "binary_unit_estimate",
      codeStructureTags: ["storage", "calculation"],
      feature: "二进制单位容量估算",
      trapType: "bit_byte_unit_confusion",
      promptDirective: "必须考 bit/Byte 与 KB/MB 的换算或容量估算，明确 8 bit = 1 Byte。",
    },
    {
      suffix: "algorithm-property",
      minGespLevel: 3,
      noiTier: "entry",
      taskFlavor: "algorithm_property_concept",
      codeStructureTags: ["algorithm", "concept"],
      feature: "算法性质判断",
      trapType: "deterministic_vs_ambiguous_step",
      promptDirective: "必须考有穷性、确定性、输入输出等算法性质，给出具体步骤而非只问定义。",
    },
    {
...
export function buildArchetypePlanForBundle(params: {
  ...
  const requiredCoverage = minimumArchetypeCountForExam(params.examType);
  if (archetypes.length < requiredCoverage) {
    throw new Error(
      `archetype coverage below ${requiredCoverage} for ${params.examType}|${params.questionType}|${params.difficulty}|${params.kpGroup}: ${archetypes.length}`,
    );
  }
