...
Schema 采用版本化迁移（`server/db/migrations/` + `scripts/maintenance.ts migrate` + `schema_migrations` 表）。Session 不落 Postgres，由 Redis 通过 `connect-redis` 存储。`__Host-Round1.sid` 为代码常量（非可配置项），不暴露到 .env。
