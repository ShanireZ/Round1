...
| `/api/v1/classes/join`、`/api/v1/coach/**`                                                                                                                                                                                                                                   | 现状契约 | 当前已挂载教练后端 slice：班级、成员、邀请、多教练、assignment 与基础报表；Coach 前端页面仍是后续目标                                                          |
...
| `GET`    | `/api/v1/auth/session`                                     | 当前浏览器会话状态；匿名返回 `authenticated=false`，供前端避免未登录时请求受保护 dashboard 数据                             | 无                       |
