...
interface ClientConfig {
  ...
  enabledAuthProviders: string[];      // ['cpplearn', 'qq']（动态）
}
```

> **当前对齐说明（2026-04-25）**：`/api/v1/config/client` 已在当前运行时挂载，但当前实际返回字段只有 `turnstileSiteKey`、`powEnabled`、`powBaseDifficulty`。`autosaveIntervalSeconds`、`examDraftTtlMinutes`、`availableExamTypes`、`availableDifficulties`、`enabledAuthProviders` 仍属于目标契约，尚未由当前 route surface 返回。
...
> **当前对齐说明（2026-04-26）**：当前运行时 surface 以 `server/app.ts` + `server/routes/*.ts` 为准。已挂载的路由组包括 `/health`、`/openapi.json`、`/config/client`、`/auth/**`、`/admin/**`，以及 Phase 11 的当前运行时 slice：`GET /api/v1/exams/catalog`、`GET /api/v1/exams/active-draft`、`POST /api/v1/exams`、`POST /api/v1/exams/:id/attempts`、`GET /api/v1/exams/:id/result`、`PATCH /api/v1/attempts/:id`、`POST /api/v1/attempts/:id/submit`、`GET /api/v1/attempts/active`、`GET /api/v1/users/me/attempts`、`GET /api/v1/users/me/stats`。其中 autosave 当前是“`X-Tab-Nonce` 校验 + `patches[]` + `jsonb_set()` 增量更新”，并带 per-user 频控；startAttempt 现已调度 BullMQ delayed auto-submit job、回写 `attempts.auto_submit_job_id`，在 `papers.status='draft'` 上做 CAS，并在任务模式下把 `assignment_progress` 从 `pending` 推到 `in_progress`；submit 当前已覆盖 finalized attempt 幂等返回、超时时落 `auto_submitted`、基础客观题与阅读/完善程序子题级聚合、`aiReportJson/reportStatus` wrongs 报告回写，以及 manual submit 时的 auto-submit job 取消；result 接口会把 `attempts` 聚合字段、`ai_report_json` 与题面/解析拼成稳定结果页 payload；`attempts/active` 已返回剩余时间与恢复路径。`/api/v1/classes/join`、`/api/v1/coach/**` 与 `/api/v1/docs` 仍未挂载。

| 接口组                                                                                                                                                                                                                                      | 状态     | 备注                                                                                                            |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | --------------------------------------------------------------------------------------------------------------- |
| `/api/v1/health`、`/api/v1/openapi.json`、`/api/v1/auth/**`、`/api/v1/admin/**`                                                                                                                                                             | 现状契约 | 当前已挂载                                                                                                      |
| `/api/v1/config/client`                                                                                                                                                                                                                     | 现状契约 | 当前已挂载，但 payload 仍是最小运行时字段                                                                       |
| `/api/v1/exams/catalog`、`/api/v1/exams/active-draft`、`POST /api/v1/exams`、`POST /api/v1/exams/:id/attempts`、`GET /api/v1/exams/:id/result`、`PATCH /api/v1/attempts/:id`、`POST /api/v1/attempts/:id/submit`、`/api/v1/attempts/active` | 现状契约 | 当前已挂载的 Phase 11 运行时接口，严格 prebuilt-only；autosave 增量保存；submit/result 已含 grouped grader 与稳定结果 payload 语义 |
| `/api/v1/users/me/**`                                                                                                                                                                                                                       | 现状契约 | 学生历史与统计已挂载，并被当前 Dashboard 消费                                                                    |
| `/api/v1/classes/join`、`/api/v1/coach/**`                                                                                                                                                                                                  | 目标契约 | 班级/教练运行时接口仍未挂载                                                                                     |
| `/api/v1/docs`                                                                                                                                                                                                                              | 目标契约 | Swagger UI 当前未挂载                                                                                           |

| 方法     | 路径                                                       | 说明                                                                                                     | 鉴权       |
| -------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ---------- |
| `GET`    | `/api/v1/health`                                           | 健康检查                                                                                                 | 无         |
| `GET`    | `/api/v1/openapi.json`                                     | OpenAPI 文档                                                                                             | 开发/admin |
