...
**托管**：项目自托管字体文件于 **Cloudflare R2**（后续迁移国内 CDN），运行时由同源 `/font/*.woff2` 代理到当前 `R2_PUBLIC_BASE_URL` 的 `/font/*.woff2` 路径，不走 Google Fonts CDN。CppLearn OIDC 横幅图片由 CppLearn 提供，上传到 `R2_PUBLIC_BASE_URL/logo/cpplearn.jpg`，页面只引用同源 `/logo/cpplearn.jpg`。
