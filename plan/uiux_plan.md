...
14. **A2UI 设计辅助 surface** — 作为 agent-facing UI renderer / `/dev/ui-gallery` 的优先设计体系，通过 Round1 token bridge 继承既有视觉系统；payload 由 basic catalog / factory 生成并经 schema 校验，覆盖已安装 basic catalog 的主要组件能力（含 media 与 modal），现有生产页面 primitive 作为受控辅助实现
