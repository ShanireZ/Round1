...
2026-04-29 二次收口补充：真实功能页的可见文案必须优先使用学生、教练、管理员能理解的中文业务语义；API 路径、runtime/payload、内部 attempt/tab 标识、owner/assignment-only 等实现词只保留在代码、测试和文档契约中，不作为页面说明文案。
...
- ② 规则说明：100 分制构成 + 按运行时/蓝图口径展示的时长 + 立即计时警示
- ③ 预制卷可用性：展示当前类型/难度可用套数；缺卷时 CTA 禁用；已有 draft 时优先提示继续草稿
- ④ 后端返回稳定 rate-limit / retry-after 时才展示倒计时；不得恢复旧在线组卷 cooldown 语义
...
- 大班级热力图和学生表必须分页/窗口化，避免一次性渲染全量 student × KP 矩阵
...
- **CoachClassDetail**：班级 Tabs（成员/邀请/教练组）+ 班级码、报告入口
- **CoachAssignments**：任务列表 + inline 新建表单（班级选择、已发布预制卷选择、截止时间、关闭任务）
- **A2UI Coach BYOC**：`Round1CoachClassSnapshot` + `Round1CoachReportSnapshot` 复用本地 Card/Badge/Progress primitive，在 `/dev/ui-gallery` 先验收 agent-facing 设计片段。
- **Coach UI 收口状态**：`CoachClassDetail` 的成员/邀请/教练组深层管理已落地，CoachClasses/CoachClassDetail/CoachAssignments/CoachReport 已进入 `ui-visual-audit.spec.ts` 桌面/移动无横向溢出验收；更丰富的教练用户搜索/选择器属于后续体验增强，不再阻塞当前功能页收口。
...
**托管**：项目自托管字体文件于 **Cloudflare R2**（后续迁移国内 CDN），运行时只引用同源 `/font/*.woff2`。开发环境由 Vite 读取 `R2_PUBLIC_BASE_URL` 代理到 R2 `/font/*.woff2`，生产环境由 `Caddyfile.example` 中的 R2 源站字面量代理到同一路径，不走 Google Fonts CDN。CppLearn OIDC 横幅图片由 CppLearn 提供，上传到 R2 `/logo/cpplearn.jpg`，页面只引用同源 `/logo/cpplearn.jpg`。
...
- **preload 关键字重**：HTML `<head>` 从同源 `/font/` preload Geist 400/500/600 + HarmonyOS 400，其他字重延迟
...
段位系统（rank-scale 色板）· AI 智能建议（Dashboard 占位区）· QQ 互联（AuthLayout 社交区可扩展）· CppLearn OIDC 横幅图片（仅用于身份入口，不替代 Round1 主品牌）· 多语言（字体栈已含西文+中文 fallback）· 真题审核工作流增强。
...
2. **`client/src/styles/globals.css`** — Light / Dark 主题 CSS 变量，字体 `@font-face`（同源 `/font/` 代理到 R2）
...
14. **A2UI 设计辅助 surface** — 作为 agent-facing UI renderer / `/dev/ui-gallery` 的优先设计体系，通过 Round1 token bridge 继承既有视觉系统；payload 由 basic catalog / factory 生成并经 schema 校验，覆盖已安装 basic catalog 的主要组件能力（含 media 与 modal），并通过 Round1 BYOC custom catalog 复用本地 primitive 渲染页面验收片段；现有生产页面 primitive 作为受控辅助实现
