...
- **AdminImports**：question bundle / paper-packs.json dry-run 结果、错误报告、导入历史
...
- `apps/web/src/styles/tokens.css` — 所有令牌 CSS 变量
...
1. **`apps/web/src/styles/tokens.css`** — 全部设计令牌（颜色 / 圆角 / 间距 / 阴影 / 字体栈 / 动效曲线）
2. **`apps/web/src/styles/globals.css`** — Light / Dark 主题 CSS 变量，字体 `@font-face`（R2 URL）
3. **`apps/web/src/components/ui/*`** — shadcn/ui 基础组件（Button / Input / Select / Card / Dialog / Toast / Tabs / Tooltip / Sheet / Dropdown / Badge / Table / Skeleton / Progress / Avatar / Switch / Checkbox / Radio）按本方案视觉规则微调
4. **`apps/web/src/components/layout/*`** — `AppShell` / `AuthLayout` / `FocusLayout` / `CeremonyLayout` / `TopBar` / `Sidebar` / `CommandBar`
5. **`apps/web/src/components/brand/*`** — `Logo`（SVG）、`HeroBackdrop`（描边大字背景）、`MeshGradient`（mesh 背景）、`NoiseTexture`（Dark 噪点）
6. **`apps/web/src/components/domain/*`** — `QuestionRenderer`（MCQ / ReadCode / FillBlank）、`CodeBlock`（Shiki 自定义主题）、`StatBadge`（AC/WA/TLE/MLE/RE）、`ExamTypeBadge`、`DifficultyBadge`、`RankHeatmap`、`AbilityRadar`、`ProgressCurve`、`ScoreReveal`（揭晓动画 + 粒子）
7. **`apps/web/src/lib/theme.ts`** — 主题切换逻辑（prefers-color-scheme + localStorage）
8. **`apps/web/src/lib/motion.ts`** — 动效预设（framer-motion variants 或 CSS 动画配置）
9. **`apps/web/src/styles/print.css`** — 印刷级打印样式
