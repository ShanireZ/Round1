...
**托管**：项目自托管字体文件于 **Cloudflare R2**（后续迁移国内 CDN），运行时由同源 `/font/*.woff2` 代理到当前 `R2_PUBLIC_BASE_URL` 的 `/font/*.woff2` 路径，不走 Google Fonts CDN。CppLearn 字标 `HYShangWeiShouShuW.woff2` 在未迁入当前 R2 前，可通过 `CPPLEARN_FONT_PUBLIC_BASE_URL` 单独配置代理源，但页面仍只引用同源 `/font/HYShangWeiShouShuW.woff2`。
...
段位系统（rank-scale 色板）· AI 智能建议（Dashboard 占位区）· QQ 互联（AuthLayout 社交区可扩展）· CppLearn OIDC `贝塔问天录` 字标（HYShangWeiShouShuW，仅用于身份入口，不替代 Round1 主品牌）· 多语言（字体栈已含西文+中文 fallback）· 真题审核工作流增强。
