...
> **GESP-6 库存补充记录（2026-05-07）**：`2026-05-07-bulk36-gesp6-default-only-gap-fill-v01` 与 `2026-05-07-bulk36-gesp6-default-only-gap-fill-v02` 已分别按 3 题/bundle 生成 12 个 GESP-6 `single_choice/medium/ALG` bundle，共 72 题；生成、两轮 LLM 审核与修复均通过 `--provider-lane default-only` 使用 `.env` 的 `LLM_PROVIDER_DEFAULT`。两轮导入 dry-run 均 12/12 通过，apply 共 72 题，duplicate content hash 均为 0。当前题目数量统计真源已迁到 `count/`，本轮刷新见 `count/state/question-inventory.*`、`count/question-counts-current.*` 与 `count/snapshots/2026-05-07-non-real-question-audit.*`；GESP-6 `single_choice/medium/ALG` 可用数从 30 增至 102，缺口从 195 降至 123。本日继续相同 `bulk36` shard 时需使用新的 `--agent-label` 或不同 pipeline label，避免 `--skip-existing` 复用已有 `bulk36-a01-b0001...` bundle 路径。
...
- [x] LLM 模拟题生产增加题型多样性蓝图与三道质量门（2026-05-07：新增 `config/questionArchetypes.ts`、生成前 archetype selector、两轮 judge 质量 rubric、导入前 diversity validator，以及 `audit-question-diversity-2026` 审计 CLI；`batch-generate-llm --plan-only` 已输出 `diversityPolicyVersion`、archetype plan 和 taskFlavor 分布。同日新增 `count/` 统计真源与 `report-question-counts`，后续题目数量、质量扣减、rewrite/archive 与缺口统计统一维护在 `count/`。）
