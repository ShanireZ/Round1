...
- Browser visual acceptance now covers Dashboard, command navigation, role-aware AppShell/mobile navigation, ExamNew, Auth entry, Account, Coach, Admin, ExamResult print/reduced-motion, A2UI BYOC gallery, and UI Gallery V2 charts/data backgrounds in `server/__tests__/e2e/ui-visual-audit.spec.ts`; the current run passed 11/11.
...
- 2026-04-29 route/FocusLayout closure pass added shared protected-route gates for logged-in, Coach, and Admin pages; lower-privilege direct links now render a user-facing 403 state. ExamSession now uses the FocusLayout header portal, adds the required mobile warning, and keeps a fixed bottom question navigator visible across desktop/mobile.
...
- [x] 对 CSP-J/S、GESP 历史题库继续按 `plan/reference-paper-audit.md` 做题面、答案、解析、知识点标签与官方答案比对复核。（2026-04-26：已完成一轮全量确定性审计与官方答案比对，修复 2 道 GESP 题面/答案/解析问题，并消除官方小写选项字母导致的比对假阳性；随后用 `scripts/review.ts review-real-papers --write` 完成覆盖 CSP-J、CSP-S、GESP 及单选/阅读程序/完善程序的 LLM/人工语义抽样复核，所有报告均无 skipped/warnings/manual_check；详见 `docs/plans/2026-04-26-real-paper-audit-followup.md`。）
