...
| 文件/目录 | 用途 |
| --- | --- |
| `docs/README.md` | docs 导航、目录职责和维护命令 |
| `docs/maintenance.md` | 文档生命周期、完成、清理、归档和问题登记规则 |
| `docs/issues-and-followups.md` | 跨计划的问题汇总、阻塞项和后续跟进登记 |
| `count/other-inventories/docs/` | `scripts/audit.ts report-docs-inventory` 生成的文档盘点结果 |
| `docs/plans/README.md` | dated plan 的状态、命名和维护入口 |

新增或关闭文档后应运行 `npm run inventory:docs -- --write` 刷新盘点。
...
文档应服务执行者。能用一句话说清的，不扩成口号；需要严谨的地方，写清字段、路径、状态、命令、验收和风险。
...
计划不必追求完整预测所有细节，但必须写清不会做什么。`Non-goals` 对防止范围漂移很重要，尤其是 UI 重设、在线组卷、运行时 LLM、不可逆迁移和新增外部服务。

计划文件顶部应尽量包含：

```text
Status: active | completed | superseded | archived | reference | blocked | deferred
Date: YYYY-MM-DD
Owner: <role/person>
Scope: <short scope>
```

没有状态头的旧计划必须出现在 `count/other-inventories/docs/docs-inventory.md` 的缺口列表中，直到被逐一补齐或登记为例外。
...
当一份文档同时包含历史记录和当前指导意义时，文件顶部必须给读者方向：继续使用、仅供参考、已被替代或部分过时。不要让执行者靠 git 历史猜。
...
Reference 应描述当前可依赖契约，不承载长篇讨论。讨论、备选方案和阶段任务放 `docs/plans/`；最终结论回写 reference。
...
- 没有把目标态写成已落地。
- 没有暴露 secret 或真实个人信息。
- Node/npm 等运行时版本口径与 [14-deployment-ops.md](14-deployment-ops.md) 的运行时工具链基线、`package.json` engines、lockfile 和 Dockerfile 一致。

纯文档变更也应检查基本可执行性：文件路径存在、命令仍在 package.json 或 scripts 中、状态值与 schema/代码一致、引用目录没有写成不存在的 `docs/plan/`。
...
- 标准目录每次重大架构决策后复核一次。
- 每次 docs 布局、计划收口、问题关闭或批量清理后刷新 `count/other-inventories/docs/`。

## 问题与情况汇总

`docs/issues-and-followups.md` 是跨计划问题的当前登记表。以下内容不得只留在单个 dated plan 中：

- 阻塞上线、导入、部署、内容生产或安全决策的问题。
- 重复出现的失败模式、误判口径或人工流程缺口。
- 已完成计划遗留的后续动作。
- 尚未形成实施计划但需要后续决策的情况。

每个登记项必须包含 ID、状态、来源、当前情况、下一步动作和关闭条件。关闭时必须填写 evidence，不能只把状态改成 closed。

## 收口与清理

计划或 follow-up 收口时按以下顺序处理：

1. 把仍然有效的长期规则回写到 `standard/`、`plan/` 或 top-level topic doc。
2. 把未完成项移入 `docs/issues-and-followups.md`，写清触发条件和关闭条件。
3. 给原 dated plan 添加 `Status:`，并说明 completed、superseded、archived、blocked 或 deferred。
4. 运行 `npm run inventory:docs -- --write`，确认缺口没有被藏起来。

清理文档时优先合并和标注状态，不优先删除。只有确认重复、无独立审计价值且不会造成历史断链的文档才可删除。
...
Backlog 项应能被关闭。每项至少有验收条件或触发条件；只有标题没有范围的条目必须补上下文或移出主 backlog。
...
面向操作的文档使用祈使句和短步骤；面向设计的文档先给结论，再给理由和取舍。避免“众所周知”“显然”“最佳实践要求”这类无法验证的表达。
...
外部链接应优先指向官方公开资料。若外部链接只是背景，不应成为 Round1 必须执行的唯一依据；必须把可执行规则落在本仓库标准中。
...
- 旧计划未完全回写 reference。
- 某功能已实现但缺截图、runbook 或验收记录。
- 某标准合理但暂时缺自动化 guard。
- 外部服务配置已变更但部署文档未演练。

文档债务应写入 backlog 或 followup，标注风险和触发收口条件。不得把会导致误操作的内容长期留在主要入口。
