# Agent 协作与工具调用规范

## 适用对象

本规范适用于 Codex、Claude、内容生产 agent、审查 agent、脚本执行 agent，以及所有通过 MCP/插件/CLI 修改仓库的自动化协作者。

## 基本原则

- 先读现状，再改文件。
- 尊重工作区已有改动；不得回滚自己没有做的修改。
- 小步执行、及时验证、记录残余风险。
- 安全、数据、考试公平、UI/UX 定稿优先于速度。
- 对库、框架、SDK、CLI、云服务问题，必须用 Context7 或官方文档确认当前用法。
- 外部大型公司规范只能作为方法参考；落地时必须改写成 Round1 的文件、命令、状态和验收规则。

## 文件修改

- 代码和文档手动编辑优先使用 `apply_patch`。
- 不用脚本批量改文件，除非变更机械且可验证。
- 不在无关文件做格式化或重排。
- 新增文件必须放入正确目录，并在索引/README/reference 中补链接。
- 写中文文档可使用中文标点；代码保持仓库格式。

## Shell 调用

- 搜索优先 `rg` / `rg --files`。
- 文件读取可并行，避免串联嘈杂输出。
- 不运行 destructive 命令，除非用户明确要求或单独批准。
- 重要命令因沙箱/网络失败时，按权限流程请求提升，不绕过审批。
- 长任务必须等完成或明确说明未完成，不能留后台关键进程。

## MCP/插件

- Context7：库/框架/SDK/API/CLI/云服务当前文档。
- Browser Use / Playwright：本地浏览器、localhost、截图、交互验证。
- GitHub：远程 issue/PR/CI 信息。
- Cloudflare：仅在明确涉及 Cloudflare 配置/API 时使用。
- Presentations/Documents/Spreadsheets：仅在对应文件类型任务使用。

使用外部资料时必须：

- 优先官方公开资料。
- 不复制大段原文。
- 明确区分事实、推断和本项目决策。
- 不把未公开内部规范、传闻或二手文章写成标准依据。
- 对会变化的库/API/云服务信息重新确认当前版本。

## 子 Agent

只有用户明确要求并行 agent、sub-agent、delegation 时才可派发。派发时必须：

- 任务自包含、边界清晰。
- 写明文件所有权。
- 告知 sub-agent 不要回滚他人改动。
- 不把当前关键阻塞任务交出去等待。

## 内容生产 Agent

- 只能在离线内容环境生成候选资产。
- 输出必须进 `artifacts/tmp/**` 或标准 bundle 路径。
- 不直接写生产 DB。
- 必须经过 schema、去重、sandbox、judge/人工复核后才可 apply。

## 调用外部服务

- 不发送 secret、session、验证码、用户隐私、未脱敏日志。
- LLM 调用必须记录 provider/model/tokens/latency/error。
- 邮件、Turnstile、OIDC、Cloudflare 等调用必须走配置真源。
- 网络失败不得静默吞掉；要返回可行动错误。

## Agent 输出规范

实现类任务最终说明必须包含：

- 改了哪些文件。
- 核心行为变化。
- 运行了哪些验证。
- 未运行验证的原因。
- 需要用户注意的风险。

审查类任务必须先列 findings，带文件和行号；无问题也要说明残余测试风险。

如果任务只改文档，最终说明仍要包含检查结果，例如路径/链接/术语扫描；不要用“无需验证”结束。

## 禁止事项

- 禁止为了让测试通过而删除有效测试。
- 禁止改 UI/UX 定稿风格。
- 禁止恢复在线组卷、在线换题、运行时 AI 生成题。
- 禁止伪造已运行测试或外部查询结果。
- 禁止在计划和文档中把未落地能力写成已完成。
- 禁止把临时 alias 当作正式可审计资产。

## Agent 执行流程

Agent 处理实现任务时必须按以下顺序：

1. 读用户最新请求。
2. 读相关 plan/standard/code。
3. 明确硬约束和风险。
4. 小步编辑。
5. 运行最小有效验证。
6. 更新文档或说明未更新原因。
7. 最终汇报文件、行为、验证、风险。

执行过程中如果发现 plan、standard、代码互相冲突：

1. 先停止扩大修改范围。
2. 找到较新的当前对齐说明或代码真源。
3. 在文档中显式写“现状契约/目标契约/legacy”。
4. 必要时新增 followup，而不是悄悄按个人判断覆盖。

## 工具选择

| 任务 | 首选 |
| --- | --- |
| 搜索文本 | `rg` |
| 查文件列表 | `rg --files` |
| 修改文件 | `apply_patch` |
| 库/框架/API 文档 | Context7 或官方 docs |
| 本地浏览器验证 | Browser Use / Playwright |
| GitHub PR/issue | GitHub MCP |
| Cloudflare API | Cloudflare MCP |
| 文档/表格/演示文件 | 对应插件 |

## Agent 上下文卫生

- 不批量读取无关大文件。
- 先读索引和 heading，再读关键片段。
- 长文档只提炼与任务相关规则。
- 不重复做已由 sub-agent 完成的工作。
- 不把外部资料大段复制进仓库。

## 多 Agent 协作

多个 agent 并行时必须划分所有权：

- 文件所有权。
- 决策所有权。
- 验证所有权。
- 汇总者。

任何 agent 都不得回滚他人改动。冲突时暂停并汇报。

## 高风险操作

以下操作必须先获得明确授权或计划：

- 删除数据或递归删除文件。
- 强制 push。
- 数据库不可逆迁移。
- 生产配置变更。
- 大规模移动/重命名内容资产。
- 修改认证、安全、权限默认值。

高风险操作的 agent 汇报必须写清回滚或恢复路径。没有恢复路径的操作，不应由 agent 在普通实现任务中顺手完成。

## Agent 自检清单

最终回复前自检：

- 是否回答最新用户请求。
- 是否只改了相关文件。
- 是否尊重已有未提交改动。
- 是否运行了该跑的验证。
- 是否说明未验证项。
- 是否提供了用户能继续行动的路径。

文档/标准类任务还要自检：

- 是否新增了和 `plan/uiux_plan.md` 冲突的视觉规则。
- 是否把目标态写成已落地。
- 是否引用了不存在路径，例如 `docs/plan/`。
- 是否把外部规范原文照搬成 Round1 条款。
