...
- 新生成的模拟题 bundle 必须携带题型多样性元数据，包括 `archetypeId`、`taskFlavor`、代码结构/容器标签、normalized template key 和质量 rubric。
...
- 官方答案 mismatch。
- 人工抽样问题数。
- archetype/taskFlavor 分布、normalized template cluster、难度 rubric 通过率和低质量候选数。
...
LLM 出题前必须从蓝图矩阵选择明确 archetype，prompt 不能只写“GESP-6 / DS / hard”。同一 bundle 的 3 题必须使用 3 个不同 archetype；同一 shard 需要记录 `taskFlavor` 分布，并在导入前由 deterministic diversity validator 拒收参数化模板近邻。

难度不能只由自然语言判断。两轮 judge 应输出 `reasoningSteps`、`stateVariables`、`conceptCount`、`traceSteps`、`trapType`、`difficultyFit` 和 `qualityScore`。hard 题若只是循环次数、最终变量值或简单 stack/queue 状态追踪，必须进入 repair/regenerate。
