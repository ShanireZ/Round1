...
- 先问“为什么需要”，再采集、展示、导出或记录数据。
- 默认最小化：列表给摘要，详情按权限单独取，日志给 hash/ID/摘要。
- 默认可解释：用户、coach、admin 看到的数据范围必须符合其角色语义。
- 默认可追溯：Admin 跨用户查看、导出、角色变更、设置变更必须能审计。
- 默认不外发：生产用户数据不进入 LLM、外部调试工具或未批准第三方服务。
...
新增字段如果不能说明使用目的、展示范围、保留策略和删除/归档方式，应先不入库。
...
Admin 可见不等于所有页面都可见。跨用户敏感数据应在明确的 Admin 审计/排障入口展示，不在普通 Dashboard、Coach 页面或前端配置中顺手下发。
...
保留期调整属于隐私与运维共同决策。缩短保留期要确认不破坏审计和排障；延长保留期要确认有明确价值且不增加不必要隐私风险。
...
用户删除不得释放邮箱/用户名给新用户复用，避免历史报表、审计记录和班级记录出现冒名风险。
...
脱敏要在日志入口统一完成，不依赖每个调用点记得手动处理。新增日志字段前应判断是否可能包含 PII、secret、题库版权原文或用户答案全文。
...
LLM 任务如需要真实内容资产，应使用离线内容环境和 runId 追踪。不得把线上用户作答、班级成员、邮箱列表或认证日志作为 prompt 上下文。
...
导出默认面向当前筛选或当前对象，不默认全量。大导出需要异步任务、过期时间、下载权限校验和审计。
...
页面截图、录屏、Sentry replay 或视觉回归材料不得包含真实邮箱、真实学生姓名、验证码、token、完整成绩导出或生产题库敏感来源。
...
隐私 incident 处理中不要为了排障扩大暴露范围。需要共享证据时优先使用脱敏样本、request id、hash 和摘要。
...
- 是否有保留、归档、删除或脱敏策略。
- 是否补了权限过滤和脱敏测试。
- 是否影响截图、导出、Sentry、LLM report 或 Admin 审计材料。
- 是否有用户删除/禁用后的展示策略。
...
- 禁止把生产用户数据复制到离线内容生成 prompt。
- 禁止在长期可访问文档中粘贴未脱敏的用户、班级、成绩或认证日志。
