...
- CppLearn OIDC 登录视觉使用横向 `贝塔问天录` 字标，字体为 `HYShangWeiShouShuW`，运行时同样通过 `/font/HYShangWeiShouShuW.woff2` 同源字体代理加载；若当前 `R2_PUBLIC_BASE_URL` 尚未包含该对象，可由 `CPPLEARN_FONT_PUBLIC_BASE_URL` 单独配置公开源，但组件不得直接写跨域字体 URL。该字标只用于 CppLearn 身份入口，不替代 Round1 主品牌。
- QQ 互联入口只在 feature flag 返回可用 provider 时显示，视觉上必须与密码/CppLearn 入口同级但不得在未实现流程时常驻主路径。
...
- 前端 `@font-face` 与 `index.html` preload 必须使用同源 `/font/*.woff2` 路径；开发和生产部署把通用字体代理到当前公开 `R2_PUBLIC_BASE_URL/font/*.woff2`，CppLearn 字标可代理到 `CPPLEARN_FONT_PUBLIC_BASE_URL/font/HYShangWeiShouShuW.woff2`。避免浏览器直接跨域请求字体。`client/public/fonts` 只作为本地缓存/备份说明，不作为运行时默认源。
- `font-display: swap`。
