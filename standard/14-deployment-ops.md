...
- 服务使用非 root 用户。
- 自动安全更新按服务器策略开启。
- Cloudflare Full Strict + Caddy TLS 上线前验证；Caddy 必须强制 HTTPS，TLS 1.2+，HTTP/2+。协议配置保持 Caddy 默认 `h1/h2/h3`；不要只配置 `h2/h3`，因为当前 `h2` 仍需要 `h1`。若启用 HTTP/3，防火墙必须同步允许 UDP 443；若只要求 HTTP/2，则显式配置 `h1/h2`。
- Caddy 配置以 `Caddyfile.example` 为模板，且模板必须使用 Caddyfile 原生语法，不使用 Caddy JSON config，也不从 `.env` 读取站点域名、静态目录、API upstream、日志路径或 R2 源站：静态资源由 Caddy 直接托管，API 动态请求反代到 Node；带哈希资源缓存 30 天，普通静态资源缓存 1 天，SPA 入口与回退不缓存；站点访问日志使用 JSON 并按 `100MiB`、`10` 份、`720h` 保留滚动。
- 因生产静态 HTML 不经过 Express Helmet，Caddy 模板必须在静态响应层设置 HSTS、CSP、`X-Content-Type-Options`、`X-Frame-Options`、`Referrer-Policy` 与 `Permissions-Policy`。
...
- 前端字体使用同源 `/font/*`：Vite 开发代理读取 `R2_PUBLIC_BASE_URL`，生产 Caddy 通过 `Caddyfile.example` 中的 R2 源站字面量代理到同一公开资源源站。CppLearn OIDC 横幅图片使用同源 `/logo/cpplearn.jpg`，同样按开发/生产代理到 R2 `/logo/cpplearn.jpg`。不要让浏览器直接跨域请求字体或品牌图片，除非资源源站已配置正确 CORS。
