...
async function inspectProblemset(examIds) {
  for (const examId of examIds) {
    ...
    console.log(
      `First problem keys: ${firstProblem ? Object.keys(firstProblem).join(", ") : "N/A"}`,
    );
    console.log(
      `First question keys: ${firstQuestion ? Object.keys(firstQuestion).join(", ") : "N/A"}`,
    );
...
async function checkAvailability(fromId, toId) {
  for (let examId = fromId; examId <= toId; examId++) {
    try {
      ...
      const problemset = data.currentData?.problemset;
      console.log(
        `${examId}\t${data.code}\t${problemset?.name ?? "N/A"}\t${problemset?.problems?.length ?? 0}`,
      );
    } catch (error) {
...
async function main() {
  ...
  if (mode === "inspect") {
    if (rest.length === 0) {
      throw new Error(
        "Usage: node scripts/commands/collect/exploreLuogu.mjs inspect <examId> [examId...]",
      );
    }
  ...
  if (mode === "availability") {
    ...
    if (Number.isNaN(from) || Number.isNaN(to)) {
      throw new Error(
        "Usage: node scripts/commands/collect/exploreLuogu.mjs availability --from <id> --to <id>",
      );
    }
