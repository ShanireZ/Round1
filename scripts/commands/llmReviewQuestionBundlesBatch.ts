...
function buildAuditPrompt(
  ...
  const instructions = [
    ...
    "- reading_program does not include sampleInputs/expectedOutputs and does not use sample input/sample output wording;",
...
function buildRepairPrompt(bundle: QuestionBundle, failedItems: AuditItemWithIndex[]) {
  ...
  return [
    ...
    "For reading_program, ensure the C++ code is deterministic, self-contained, has no stdin dependency, has empty sampleInputs/expectedOutputs, and the answer follows from tracing the actual code.",
