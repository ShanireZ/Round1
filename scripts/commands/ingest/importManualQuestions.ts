...
function requireCliArg(value: string | undefined, name: string): string {
  if (!value) {
    console.error(
      `Usage: npx tsx scripts/commands/ingest/importManualQuestions.ts <filePath> --question-type <type> --exam-type <examType> --primary-kp-id <id> [--imported-by <userId>] (missing ${name})`,
    );
    process.exit(1);
  }
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < rest.length; i += 2) {
    ...
    if (!key?.startsWith("--") || !value) {
      console.error(
        "Usage: npx tsx scripts/commands/ingest/importManualQuestions.ts <filePath> --question-type <type> --exam-type <examType> --primary-kp-id <id> [--imported-by <userId>]",
      );
      process.exit(1);
    }
