...
const usage = `Usage: npx tsx scripts/commands/audit/buildSimilarityReviewShards2026.ts --audit <similarity-audit.json> --out-dir <dir> [--shards 8]`;
    ...
    `${JSON.stringify(
      {
        ...
        uniqueItemCount: new Set(audit.pairs.flatMap((pair) => [pair.left.id, pair.right.id])).size,
        shardFiles,
      },
      null,
      2,
