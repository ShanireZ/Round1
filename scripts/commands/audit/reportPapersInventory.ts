...
function duplicateGroups(locationsByHash: Map<string, DuplicateLocation[]>): DuplicateGroup[] {
  return [...locationsByHash.entries()]
    ...
    .sort(
      (left, right) =>
        right.count - left.count || left.contentHash.localeCompare(right.contentHash),
    );
}
...
async function inventoryGeneratedYear(
  yearDir: string,
  generatedAt: string,
): Promise<GeneratedYearInventory> {
  ...
  const runDirs = fs.existsSync(yearDir)
    ...
    : [];
  const jsonFiles = listJsonFiles(yearDir).filter(
    (file) => !repoPath(file).includes("/_inventory/"),
  );
...
async function inventoryRealPapers(
  realPapersDir: string,
  generatedAt: string,
): Promise<RealPaperInventory> {
      ...
      questions.forEach((question, itemIndex) => {
        const record =
          typeof question === "object" && question !== null
            ...
            : {};
        const questionType =
          typeof record.questionType === "string" ? record.questionType : "unknown";
        const difficulty = typeof record.difficulty === "string" ? record.difficulty : "unknown";
        const primaryKpCode =
          typeof record.primaryKpCode === "string" ? record.primaryKpCode : "unknown";
...
function renderSectionMarkdown(section: SectionInventory) {
  ...
  for (const [name, counts] of Object.entries(section.counts)) {
    ...
    for (const [key, count] of Object.entries(counts).sort(([left], [right]) =>
...
async function main() {
  ...
  if (args.write) {
    ...
    await writeFile(
      path.join(outDir, "papers-inventory.json"),
    ...
    for (const section of sections) {
      ...
      await writeFile(
        path.join(sectionsDir, `${base}.json`),
