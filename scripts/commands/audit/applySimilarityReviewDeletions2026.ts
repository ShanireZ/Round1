...
const usage = `Usage: npx tsx scripts/commands/audit/applySimilarityReviewDeletions2026.ts --verdict-dir <dir> [--source-dir papers/2026] [--report-dir <dir>] [--apply] [--skip-db] [--delete-paper-slots]`;
  ...
  return {
    ...
    decisions: record.decisions.map((entry, index) => {
      ...
      return {
        ...
        deleteItemIds: normalizeStringArray(
          decision.deleteItemIds,
          `decision ${index}.deleteItemIds`,
        ),
        confidence:
...
async function loadDeletionDecisions(verdictDir: string) {
  ...
  for (const shardPath of shardFiles) {
    ...
    const componentsById = new Map(
      shard.components.map((component) => [component.componentId, component]),
    );
    const decisionsByComponent = new Map(
      verdict.decisions.map((decision) => [decision.componentId, decision]),
    );
    ...
    for (const component of shard.components) {
      ...
      if (!decision) {
        validationErrors.push(
          `${repoPath(verdictPath)} missing decision for ${component.componentId}`,
        );
        continue;
      }
      ...
      if (overlap.length > 0) {
        validationErrors.push(
          `${component.componentId} has keep/delete overlap: ${overlap.join(", ")}`,
        );
      }
      if (unknown.length > 0) {
        validationErrors.push(
          `${component.componentId} mentions unknown items: ${unknown.join(", ")}`,
        );
      }
      ...
      if (missing.length > 0) {
        validationErrors.push(
          `${component.componentId} does not partition all items: ${missing.join(", ")}`,
        );
      }
    ...
    for (const decision of verdict.decisions) {
      if (!componentsById.has(decision.componentId)) {
        validationErrors.push(
          `${repoPath(verdictPath)} has unknown component ${decision.componentId}`,
        );
      }
    }
  }
...
async function loadBundle(filePath: string): Promise<QuestionBundle> {
  ...
  if (!parsed.success) {
    throw new Error(
      `${repoPath(filePath)} is not a valid question bundle: ${parsed.error.issues[0]?.message}`,
    );
  }
  return parsed.data;
}
...
function buildDeletionPlan(params: {
  ...
  return {
    rewrites,
    deletedItems,
    deletedContentHashes: [...deletedContentHashes].sort((left, right) =>
      left.localeCompare(right),
    ),
  ...
  try {
    ...
    const counts = {
      ...
      questionExamTypes: Number(
        (
          await client.query(
            "delete from question_exam_types where question_id = any($1::uuid[])",
            [params.ids],
          )
        ).rowCount ?? 0,
      ),
      ...
      prebuiltPaperSlots: Number(
        (
          await client.query(
            "delete from prebuilt_paper_slots where question_id = any($1::uuid[])",
            [params.ids],
          )
        ).rowCount ?? 0,
      ),
    ...
    counts.questions = Number(
      (await client.query("delete from questions where id = any($1::uuid[])", [params.ids]))
        .rowCount ?? 0,
    );
...
async function writePlanReport(params: {
  ...
  const tsvPath = path.join(
    params.args.reportDir,
    `similarity-cleanup-${suffix}__deleted-items.tsv`,
  );
  const tsv = [
    [
      "itemId",
