...
const usage = `Usage: npx tsx scripts/commands/audit/auditQuestionBundleSimilarity2026.ts (--manifest <manifest.json> | --dir papers/2026) [--threshold 0.85] [--out-dir <dir>] [--cross-kp] [--max-pairs <count>] [--preview-chars 120] [--reviewed-keep-verdict-dir <dir>]`;
...
function classifyPair(params: {
  ...
  if (
    isLikelyParameterizedTemplateSimilarity(params.questionType, params.leftText, params.rightText)
  ) {
...
function resolveBundleFiles(args: AuditArgs) {
  if (args.manifestPath) {
    return readManifestBundlePaths(path.resolve(process.cwd(), args.manifestPath)).sort(
...
function auditGroup(records: AuditRecord[], args: AuditArgs) {
  ...
  for (const [currentIndex, current] of records.entries()) {
    ...
    for (const [previousIndex, intersection] of overlapCounts) {
      ...
      const similarity =
        previous.grams.size === 0 && current.grams.size === 0
          ? 1
          : union === 0
            ? 0
            : intersection / union;
...
function loadReviewedKeepPairKeys(verdictDirs: string[]) {
  ...
  for (const verdictDir of verdictDirs) {
    ...
    for (const verdictFile of listVerdictFiles(absoluteVerdictDir)) {
      ...
      ) as SimilarityReviewVerdictFile;
...
function renderMarkdown(params: {
  ...
  if (params.pairs.length > params.maxRows) {
    lines.push(
      "",
      `Markdown table truncated to ${params.maxRows} rows. See JSON for full details.`,
    );
  }
...
function defaultOutDir(args: AuditArgs) {
  ...
  return path.resolve(process.cwd(), "count/audits/similarity", thresholdSlug);
}
...
async function main() {
  ...
  const ignoredFalsePositivesPath = path.join(
    outDir,
    `${baseName}__ignored-likely-false-positive.tsv`,
  );
  ...
  const truncatedPairs =
    args.maxPairs !== undefined && pairs.length > args.maxPairs
      ? pairs.slice(0, args.maxPairs)
      : pairs;
  ...
  fs.writeFileSync(
    manualReviewCandidatesPath,
    renderHighSimilarityPairsTsv(manualReviewCandidates),
  );
