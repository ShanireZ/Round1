...
const usage = `Usage: npx tsx scripts/reportDocsInventory.ts [--source-dir docs] [--out-dir docs/_inventory] [--write] [--strict]`;
...
function renderMarkdown(params: { generatedAt: string; sourceDir: string; entries: DocsInventoryEntry[] }) {
...
async function main() {
  ...
  if (args.write) {
    ...
    await writeFile(path.join(outDir, "docs-inventory.json"), `${JSON.stringify(payload, null, 2)}\n`);
  ...
  if (args.strict && payload.summary.filesWithoutStatusHeader > 0) {
    throw new Error(`Docs inventory strict check failed: ${payload.summary.filesWithoutStatusHeader} file(s) lack a status header`);
  }
}
