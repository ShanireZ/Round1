 ...
 *
 * 用法：
 *   npx tsx scripts/ingestRealPapers.ts --dir papers/real-papers/csp-j [--skip-ai-review] [--limit 10]
 *
 * 导入流程：
 * 1. 解析 JSON → Zod 校验
 * 2. 入库 questions (source='real_paper', status='draft')
 * 3. 创建 question_reviews (review_status='pending')
 * 4. LLM 独立解题验证 → 与官方答案比对
...
const USAGE =
  "Usage: npx tsx scripts/ingestRealPapers.ts --dir <path> [--confirm] [--skip-ai-review] [--limit count] [--timeout ms]";
